...
vid_out = cv2.VideoWriter(
    'Output\\SolidWhiteRightOutput.avi', cv2.VideoWriter_fourcc(*'DIVX'), 15, size)
...
# create the final output video
vid_out = cv2.VideoWriter(
    'Output\\SolidYellowLeftOutput.avi', cv2.VideoWriter_fourcc(*'DIVX'), 15, size)
for i in range(len(img_array)):
    vid_out.write(img_array[i])
vid_out.release()
...
# create the final output video
vid_out = cv2.VideoWriter(
    'Output\\ChallengeOutput.avi', cv2.VideoWriter_fourcc(*'DIVX'), 15, size)
for i in range(len(img_array)):
    vid_out.write(img_array[i])
vid_out.release()
